...
function requireAuth(req, res, next) {
  ...
  try {
    req.user = jwt.verify(auth.slice(7), JWT_SECRET, { algorithms: ['HS256'], issuer: 'propscribe' })
    next()
  } catch (err) {
...
function signToken(payload) {
  return jwt.sign(payload, JWT_SECRET, { expiresIn: '30d', algorithm: 'HS256', issuer: 'propscribe' })
}
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  ...
  res.status(201).json({ token, user: { id: result.lastInsertRowid, email, name, plan: 'free' } })
})
...
app.post('/api/generate', requireAuth, genLimiter, async (req, res) => {
  ...
  if (usage.count >= limit) {
    return res.status(429).json({ error: 'Monthly limit reached (' + limit + ' generations). Please upgrade.', upgrade: true })
  }
  ...
  const features    = Array.isArray(req.body.features) ? req.body.features.map(f => sanitize(f, 100)).slice(0, 20) : []
  const platforms   = Array.isArray(req.body.platforms) ? req.body.platforms.map(p => sanitize(p, 50)).slice(0, 10) : []
...
app.get('/api/plans', (req, res) => {
  ...
  const plans = db.prepare('SELECT name, monthly_limit, price_naira, description FROM plans ORDER BY monthly_limit').all()
  res.json(plans)
})
...
app.listen(PORT, () => {
  console.log('\nPropScribe running at http://localhost:' + PORT)
...
function buildPrompt({ propType, listingType, location, price, size, features, extra, tone, platforms }) {
  ...
  // Build listing-type specific instructions so the AI never confuses them
  ...
  prompt += '⚠️ CRITICAL INSTRUCTION: ' + listingNote + '\n\n'
  prompt += 'PROPERTY DETAILS:\n'
  prompt += '- Type: ' + propType + '\n'
  prompt += '- Listing Type: ' + listingType + ' (THIS IS THE MOST IMPORTANT DETAIL — write ONLY for this listing type)\n'
